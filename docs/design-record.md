...
| D-115 | Narration length (D-11) is a word range from the beat's weight, scaled by the campaign's `narrationLength`. A **routine** beat is 60–120 words (§12); a **dramatic** beat — a miss, a match, a momentum burn, or a chain into a suffer move — is 120–200. `shorter` scales both ranges by 0.6 and `longer` by 1.5. Weights for scene openings, recaps and summaries land with the groups that write them (8.x, 9.x). | 17 | Approved |
