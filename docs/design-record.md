...
1. **The default checker model can't take the narrator's request.** A live probe showed `claude-haiku-4-5` rejects both adaptive thinking ("not supported on this model") and `output_config.effort` ("does not support the effort parameter"). It accepts structured output and the fallback beta, and answers in about 1 s. The Claude implementation therefore knows per model whether to send thinking and effort. That stays inside `claude.ts`; D-112's interface doesn't change, and a caller never passes model knowledge.
2. **Every checker failure fails closed and pauses play.** The failure is `ai.failed` under the checker's name and model, with the checker's own error kind. A `rejected` request, meaning a misconfigured `ASTROLABE_CHECK_MODEL`, reads as a server configuration problem rather than an outage, and Retry works once it is fixed.
3. **Input shape.** The checker takes the passage's text, plus its tagged segments when there are any. Beat narration has segments. A 7.9 rewrite streams as prose, and an injury is one sentence.
4. **Violation shape.** Each violation carries a `rule`: `undeclared_action`, `player_interior`, `voice` or `injury`. It also carries the `character` it concerns, the segment index when there is one, the verbatim quote, and `why`. A quote counts as verbatim when it appears exactly in that segment's text, or in the passage's text when there is no segment. The undeclared-action rule judges the whole passage, whatever the labels: an action inside a Full-voice line of speech is still an action (D-127's amendment).
5. **Withdrawal record.** `narration.withdrawn` also records the `role` (`beat`, `revision` or `injury`) and, for a revision, the `targetEventId` it was rewriting. `checker` is `segment_checks` or `authority_check`, and an authority check also records the checker's `model`. A segment-check failure's violation has the rule `segment_check`, with its problem in words. A checker failure over text that was already shown is withdrawn with the rule `unchecked`, because provisional text is never quietly removed either.
6. **Cut-off and unreadable output.** A reply cut off at `max_tokens`, or output that won't parse, is not an authority finding. It keeps 7.5's silent `reset` and re-ask.
7. **One command.** The withdrawals are written in the same command as the outcome, alongside every attempt's `ai.completed`, the narrator's and the checker's. The client sees each withdrawal the moment it happens, as a `withdrawn` frame carrying the reason and the rejected text. A `checking` frame marks when generation ends and the check begins.
8. **Unsaved proposals.** A harm proposal isn't shown until checked, so a failing injury is re-asked without anything to strike on screen. It is still recorded as a withdrawal with `role: 'injury'`, so the automated rejection stays traceable.
9. **The reason in words** comes from the rule and character by one shared function, so the frame and the log say the same thing.
10. **Live matrix.** The matrix uses five beats that can be built today: Beat 7's chain, a standalone Endure Harm with nothing declared, a Gather Information weak hit, an aided scan upgraded by a momentum burn (Beat 5), and a Face Danger strong hit. Beats needing complications, oracle grounding or clocks wait for groups 8 and 9. | 20, 21 | Approved (amended round 21) |
