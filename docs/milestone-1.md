...
**Finding, now fixed as D-156: an established fact wasn't checked against a new one.** Beat 6's Fletcher Hunt introduces himself as "forty years resident, sole survivor." Beat 7's world plan then asked "Is the knocking in the dark plating made by another living person aboard the station?", rolled Yes, and Beat 8's interpret call established Kwan Kaan — alive, aboard, no reroll considered. The gap wasn't in D-138's design — its prompt already told the plan and interpret calls to check "the recent narration" — it was in the code: `renderState` is deliberately bounded to projected structure (7.4), so an NPC's own claim, spoken only in narrated dialogue, never actually reached either call. Fixed: `WorldBeat` now carries `recentNarration`, every earlier live passage of the current session, and both requests render it as `<recent_narration>`. Covered by a new test in `world-commands.test.ts`; full suite, typecheck and lint stay clean (1050 passed, 1 skipped). Not re-run live to confirm against real AI — the fix is mechanical (the data simply wasn't there before; now it is) and the golden session's own scope doesn't call for a second live pass just to watch this one thing.
