...
Every task in Milestone 1's list is now done or moved out (9.2, D-71). What the task list can't check is the milestone's own definition of done: the golden session played against a real campaign with real AI narration. That is a live play-through — see below.

---

## The live play-through (D-56)

The golden session was played beat by beat in the browser against the `session-1` fixture campaign (fresh, session 1 closed, nothing of session 2 begun), the dev server on real rules data and Postgres, and `ASTROLABE_AI_PROVIDER` unset — the live Claude provider, `cryptoRandomSource` dice, narrator `claude-opus-5`, planner and checker `claude-sonnet-5`. This is the milestone's own definition of done (D-56), not something the automated golden-session test can stand in for: that test runs on a stub and loaded dice (D-152).

**Real dice mean the golden session's specific numbers can't be reproduced, and the run doesn't try to.** Where a roll didn't land on the outcome a beat is written around, the policy was: void-and-redo when the beat's unique, otherwise-unexercised feature depends on that outcome; otherwise accept the roll and note the branch as covered elsewhere. Both are visible in the log.

**Beat by beat:**
- **1 — Recap.** Begin Session produced a "previously on" recap built from session 1's log. Matched.
- **2 — Frame the scene.** Derelict oracles rolled and shown as chips before the prose; the frame ended on the stakes with no nudge. Matched.
- **3 — Juno, Gather Information.** A genuine weak hit. "Give me options" offered three AI complications, each Action+Theme tagged; picked one. The narration cited the complication's facts. An unscripted bonus: the world pass asked an oracle question and narrated a short follow-up passage grounded in the answer — a real instance of the D-138 amendment's after-the-passage world flow, not exercised this way by the fixture.
- **4 — What now?** Three suggestions, each anchored in current state (Juno's logs, Vesna's sensors, Rook's entry). Combined two, as the script does. Matched.
- **5 — Rook aids Vesna.** Rook's Secure an Advantage came back a **weak hit**, not the fixture's strong hit — which correctly surfaced D-61's weak-hit choice UI (take +2 momentum, or +1 on the next move) instead of the strong-hit's automatic both-benefits. Took +1 on Vesna's next move. Vesna's Gather Information then **missed**; voided and redid it (Beat 5's unique test — the momentum-burn upsell — isn't exercised by a miss, and the miss/Pay-the-Price path is already covered live in Beat 7). The redo came back a weak hit, the burn offer appeared exactly as designed ("Burn momentum to upgrade to a strong hit? Momentum resets to +2"), and burning it flipped to a strong hit.
- **6 — A new face.** The world pass rolled character oracles and established an NPC, **Fletcher Hunt**, badged AI-established in the rail. No reroll fired this run (that's conditional on the interpret call finding a contradiction, not guaranteed) — ownership, oracle grounding, and provenance still all checked out.
- **7 — Forcing the bulkhead.** Rook's Face Danger with +edge (the scripted mistake) came back a strong hit; voided with a reason ("wrong stat") and redone with +iron. The redo also came back a strong hit (with a match) rather than the fixture's miss. **This run's Face Danger never missed, so Pay the Price → Endure Harm didn't fire live.** That chain is already deterministically verified by `golden-session.test.ts` (D-152); nothing new would be learned by voiding a correctly-stated roll purely to force a different outcome, so it was accepted. The wrong-stat void-and-redo — Beat 7's other tested feature — did fire live, with a real narrated consequence (the bulkhead gives) and a real follow-up world pass.
- **8 — Pressure.** The same world pass that resolved Beat 7 also created a tension clock, **"Something in the plating draws nearer,"** ticked to 2/6 with a reason ("the bulkhead breach let the knocking answer back"), and established a second NPC, **Kwan Kaan**, guarding the passage — described as living. Unscripted, and a clean instance of the same D-138/D-145 machinery Beat 8 exists to test. **But see the finding below:** Beat 6's Fletcher Hunt had just called himself "sole survivor," and nothing in this pass caught the contradiction.
- **9 — Corrections.** Flagged Kwan's line for tone (asked for wary and formal, not cold); the AI rewrote just that passage in place, logged as a correction. Juno's momentum was manually overridden +4 → +5 with a reason, logged as an edit. Matched, though the corrected detail was tone rather than the fixture's "shaken" — the real narration never used an interiority word for the checker to catch, so there was nothing to correct on that specific ground.
- **10 — End session.** End Session produced a real summary and open threads (who purged the logs, Kwan's question, the clock, the flight recorder), plus the Reach a Milestone reminder. Ended unedited. Matched.

**Numbers.** 154 events on the campaign; 47 `ai.completed` calls total — 10 from the instant, sub-millisecond `session-1` seed (stub) and **37 live** (narrator, planner, checker) from the play itself; **zero `ai.failed`, zero `narration.withdrawn`** — all 11 checked artifacts (9 `narration.written` + 1 `narration.revised` + 1 `session.summary_proposed`) passed D-128's checker on the first `narration_check`, no re-asks. Two legitimate voids (Beat 5's miss, Beat 7's wrong stat), one narration revision (Beat 9), one manual override (Beat 9). Total session cost 117k tokens.

**A18: held, on real data.** `ai.completed` turns out to carry `firstTokenMs` as well as total `latencyMs` — missed on the first pass through this data, corrected here. Across every player-facing passage (recap, scene frame, three beat narrations, two world passages, the Beat 9 revision), first text arrived in **1.6–2.9s**, every one inside the 5-second target, with no relation to the total completion time. Beat 7's redo, for instance, took 26.7s to fully complete and check (506 output tokens, the longest passage of the run) but its first token still landed at 2.2s — the long tail is generation-plus-check time after the player is already reading, exactly as D-128's provisional-streaming design intends. `narration_check`, `world_plan`, `trigger_check` and similar background calls carry no `firstTokenMs`, since nothing streams to the player from them.

**Finding: an established fact wasn't checked against a new one.** Beat 6's Fletcher Hunt introduces himself as "forty years resident, sole survivor." Beat 7's world plan then asked "Is the knocking in the dark plating made by another living person aboard the station?", rolled Yes, and Beat 8's interpret call established Kwan Kaan — alive, aboard, no reroll considered. The event log shows no `world_interpret` reroll and no checker involvement: D-128's checker explicitly judges only its own rubric (undeclared action, player interiority, voice, injury), never general consistency with committed narrative, and D-138's reroll step catches a roll that contradicts *established facts*, not a claim one NPC made in dialogue. Whether Fletcher's "sole survivor" line should count as an established fact the world pass must respect, or is just an unreliable NPC's own word (in which case nothing is wrong here — Fletcher lied, which is a fine beat on its own), isn't settled by any existing decision. Flagging as an open question rather than deciding it here.

**Two open items for the user, not folded in as decisions:**
1. **The void-and-redo policy this run used** — force a redo only when the beat's own unique, otherwise-unexercised feature depends on the outcome (Beat 5's burn); otherwise accept the roll as-landed and rely on the deterministic fixture test for the untaken branch (Beat 7's miss never fired live, so Pay the Price → Endure Harm ran zero times against real AI this session, only in `golden-session.test.ts`). This is a methodology call, not a design decision, and it should be a decision with an ID if it's going to govern how this validation gets re-run.
2. **The Fletcher/Kwan contradiction above** — whether it's a gap worth a decision, or working as intended.

**What this run shows plainly:** every capability the golden session exists to prove — player-chosen and AI-suggested moves, oracle-grounded scene framing, AI-owned NPCs and clocks with visible provenance, the D-61 aid choice, the momentum-burn upsell, void-and-redo, narration correction, manual override, session summary with open threads — ran live against real rules data and real AI narration, in the browser, with A18 held on every measured passage and zero failures. Whether that closes D-56 given the open items above is the user's call.
